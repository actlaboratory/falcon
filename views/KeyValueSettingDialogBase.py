    ...
    def InstallControls(self):
        ...
        self.hListCtrl, static = self.creator.listCtrl(
            _("現在の登録内容"), proportion=0, sizerFlag=wx.ALL | wx.ALIGN_CENTER_HORIZONTAL, size=(
                750, 300), style=wx.LC_REPORT | wx.LC_NO_HEADER | wx.LC_SINGLE_SEL)
        ...
        # 処理ボタン
        self.creator = views.ViewCreator.ViewCreator(
            1, self.panel, self.creator.GetSizer(), wx.HORIZONTAL, 20, "", wx.ALIGN_RIGHT)
        ...
        for i, name in enumerate(self.valueNames):
            if name[1]:
                self.edits[i], dummy = self.creator.inputbox(
                    name[0], x=500, defaultValue=self.values[i])
            else:
                self.edits[i], dummy = self.creator.inputbox(
                    name[0], x=500, defaultValue=self.values[i], style=wx.TE_READONLY)
            if name[1] is None:
                dummy.Hide()
                self.edits[i].Hide()
            if self.buttons[i]:
                dummy = self.creator.button(
                    self.buttons[i][0],
                    self.buttons[i][1],
                    sizerFlag=wx.ALIGN_RIGHT)
