...
class Dialog(BaseDialog):
    ...
    def InstallControls(self, dic):
        ...
        self.creator = views.ViewCreator.ViewCreator(
            self.viewMode, self.panel, self.sizer, views.ViewCreator.FlexGridSizer, 20)

        if dic:
            for title, content in dic.items():
                self.add(title, content)

        self.buttonArea = views.ViewCreator.ViewCreator(
    ...
    def add(self, title, content):
        if content != self.CALCURATING:
            f, static = self.creator.inputbox(
                title, x=400, defaultValue=str(content), style=wx.TE_READONLY)
        else:
            f, static = self.creator.inputbox(
                title, x=400, defaultValue=_("計算中"), style=wx.TE_READONLY)
