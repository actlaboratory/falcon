...
class virtualListCtrl(listCtrlBase.listCtrl):
    ...
    def __init__(self, *pArg, **kArg):
        lPArg = list(pArg)
        if "style" in kArg:
            kArg["style"] = kArg["style"] | wx.LC_REPORT | wx.LC_VIRTUAL
        elif len(lPArg) >= 5:
            lPArg[4] = lPArg[4] | wx.LC_REPORT | wx.LC_VIRTUAL
        else:
    ...
    def SetItem(self, index, column=0, label=None, imageId=-1):
        if not isinstance(
                index,
                int) or label is None or not isinstance(
                label,
                str) or imageId != -1:
            raise NotImplementedError
        if column < 0:
    ...
    def onLabelEditEnd(self, event):
        ...
        if (not event.IsEditCancelled()) and event.IsAllowed():
            self.lst[self.GetFocusedItem()][self.getColFromWx(
                0).col] = self.GetEditControl().GetLineText(0)
    ...
    def onColumnDragEnd(self, event):
        ...
        if event.IsAllowed():
            self.getCol(
                event.GetColumn()).width = super().GetColumnWidth(
                self.getCol(
                    event.GetColumn()).wx_col)
    ...
    def extend(self, iterable):
        ...
        if len(iterable) > 0:
            self.RefreshItems(newLen - len(iterable), newLen - 1)
    ...
    def __delitem__(self, key):
        if len(self.lst[key]) >= 500:  # 大量処理の高速化
            ...
            if self.GetFocusedItem() >= 0:
                f = self.lst[self.GetFocusedItem()]
            else:
                f = None
            top = self.GetTopItem()
            self.Focus(0)
    ...
    def __iadd__(self, other):
        ...
        super().SetItemCount(newLen)
        if oldLen < newLen:
            self.RefreshItems(oldLen, newLen - 1)
        return self
        ...
        if obj in self.lst:
            ...
            for o in reversed(previousL):
                if o in self.lst:
                    ...
                    break
        if self.GetFocusedItem() == len(self.lst) - 1:
            newFocus = self.GetFocusedItem()
        else:
            newFocus = self.GetFocusedItem() + 1
        newTop = topItem - (fId - newFocus) + self.GetCountPerPage() - 1
        if newTop >= len(self.lst):
            self.Focus(len(self.lst) - 1)
        else:
            self.Focus(newTop)
        self.Focus(newFocus)
    ...
    def AppendColumn(self, heading, format=wx.LIST_FORMAT_LEFT, width=-1):
        ...
        ret = Column(len(self.columns), result, super(
        ).GetColumnOrder(result), format, width, heading)
    ...
    def InsertColumn(
        ...
        if col == 0:
            ...
            insertedColumn = Column(
                self.GetColumnCount(),
                GetShowingColumnCount(),
                GetShowingColumnCount(),
                format,
                width,
                heading)
        for i in [j for j in self.columns if j.col >= insertedColumn.col]:
            i.col += 1
        for i in [j for j in self.columns if j.wx_col >=
                  insertedColumn.wx_col]:
            i.wx_col += 1
        for i in [j for j in self.columns if j.disp_col >=
                  insertedColumn.disp_col]:
            i.disp_col += 1
        super().InsertColumn(insertedColumn.wx_col, heading, format, width)
        self.columns.append(insertedColumn)
        for i in self.lst:
            i.insert(insertedColumn.col, "")
        return insertedColumn.col

    def DeleteColumn(self, col):
        removedColumn = self.getCol(col)
        for i in [j for j in self.columns if j.col > removedColumn.col]:
            i.col -= 1
        for i in [j for j in self.columns if j.wx_col > removedColumn.wx_col]:
            i.wx_col -= 1
        for i in [j for j in self.columns if j.disp_col >
                  removedColumn.disp_col]:
            i.disp_col -= 1
        for i in self.lst:
            del i[col]
    ...
    def Bind(self, event, handler, source=None, id=wx.ID_ANY, id2=wx.ID_ANY):
        if not isinstance(
                event,
                wx.PyEventBinder) or source is not None or id != wx.ID_ANY or id2 != wx.ID_ANY:
            raise NotImplementedError
        if event in (
