...
class clearSlider(sliderBase.slider):
    ...
    def __init__(self, *pArg, **kwargs):
        ...
        if style != 0:
            if (
                (
                    style & wx.SL_VERTICAL == wx.SL_VERTICAL) or (
                    style & wx.SL_AUTOTICKS == wx.SL_AUTOTICKS) or (
                    style & wx.SL_MIN_MAX_LABELS == wx.SL_MIN_MAX_LABELS) or (
                    style & wx.SL_VALUE_LABEL == wx.SL_VALUE_LABEL) or (
                        style & wx.SL_LEFT == wx.SL_LEFT) or (
                            style & wx.SL_RIGHT == wx.SL_RIGHT) or (
                                style & wx.SL_TOP == wx.SL_TOP) or (
                                    style & wx.SL_BOTTOM == wx.SL_BOTTOM) or (
                                        style & wx.SL_BOTH == wx.SL_BOTH) or (
                                            style & wx.SL_SELRANGE == wx.SL_SELRANGE)):
                raise ValueError(
                    "ClearSlider is not support tick, label, and Lange style.")
    ...
    def _getVarLength(self):
        return self.GetSize().GetWidth() - self.BORDER_WIDTH - self.getLeftMargin() - \
            self.getRightMargin() - self.ELLIPSE_WIDTH
