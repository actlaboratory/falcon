...
class updateDialog(BaseDialog):
    ...
    def InstallControls(self):
        ...
        if self.info["code"] == 200:
            ...
            self.gauge.Disable()
            self.infoEdit, self.infoEditStatic = self.creator.inputbox(_("version %s アップデート情報") % (
                self.info["update_version"]), defaultValue=self.info["update_description"], x=600, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.BORDER_RAISED)
            self.infoEdit.SetMinSize((600, 200))
            self.infoEdit.SetMaxSize((600, 200))
            creator = views.ViewCreator.ViewCreator(self.viewMode, self.panel, self.creator.GetSizer(),
                                                    wx.HORIZONTAL, 20, style=wx.ALIGN_RIGHT | wx.TOP, margin=20)
            self.startBtn = creator.button(_("アップデート"), self.run_update)
        elif self.info["code"] == 205:
            self.infoEdit, self.infoEditStatic = self.creator.inputbox(
                _("緊急のお知らせがあります。"), defaultValue=self.info["info_description"], style=wx.TE_MULTILINE | wx.TE_READONLY | wx.BORDER_RAISED)
