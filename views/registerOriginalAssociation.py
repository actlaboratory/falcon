...
class SettingDialog(views.KeyValueSettingDialogBase.SettingDialogBase):
    ...
    def __init__(self, parent, extention="", path=""):
        super().__init__(parent, ((_("拡張子"), "<default_" not in extention),
                                  (_("実行ファイル名"), True)), (None, (_("参照"), self.getRef)), extention, path)
    ...
    def getRef(self, event):
        ...
        d = wx.FileDialog(
            self.wnd,
            "実行ファイルの選択",
            wildcard="実行ファイル(" + pathext + ")",
            style=wx.FD_FILE_MUST_EXIST | wx.FD_SHOW_HIDDEN)
        if d.ShowModal() == wx.ID_CANCEL:
            return
        self.edits[1].SetValue(d.GetPath())

    def Validation(self, event):
        if self.edits[0].GetLineText(
                0) == "" or self.edits[1].GetLineText(0) == "":
            return
        ...
        if not misc.GetExecutableState(path):
            ...
            return
        if self.edits[0].GetLineText(
                0) == "<default_file>" or self.edits[0].GetLineText(0) == "<default_dir>":
            event.Skip()
            return
