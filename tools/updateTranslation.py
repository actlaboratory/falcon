...
if not os.path.exists(
        "tools/xgettext.exe") or not os.path.exists("tools/msgmerge.exe"):
...
subprocess.call(
    ("tools\\xgettext.exe -p locale --from-code utf-8 --package-name=Falcon %s" %
     (" ".join(files))).split())
for l in langs:
    if os.path.exists("locale/%s/LC_MESSAGES/messages.po" % (l)):
        print("Merging %s" % l)
        subprocess.call(
            ("tools\\msgmerge.exe -U locale/%s/LC_MESSAGES/messages.po locale/messages.po" %
             (l)).split())
    else:
        print("Creating %s" % l)
